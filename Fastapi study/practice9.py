...
def upload_file1(file: bytes = File(...)):
    with open('./data/file.jpg', 'wb') as f:
        f.write(file)
    return {"msg": "文件上传成功"} # File uploaded successfully


@app.post('/upload2/')
async def upload_file2(file: UploadFile):
    async with aiofile.open(f'./data/{file.filename}', 'wb') as f:
        chunk =await file.read(1024*1024)  # 读取1MB写入文件
        while chunk:
            f.write(chunk)  # 读取1MB写入文件
            chunk =await file.read(1024*1024)  # 读取1MB写入文件
...
if __name__ == '__main__':
    uvicorn.run('main13:app', host="127.0.0.1", port=8000)